...
export default function VenueManagerVenues() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedVenue) {
        await api.put(`/manager/venues/${selectedVenue.id}`, formData);
        toast.success('Venue updated successfully');
      } else {
        await api.post('/manager/venues', formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this venue?')) {
      try {
        await api.delete(`/manager/venues/${id}`);
